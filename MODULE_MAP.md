...
| 5225 | **S10** upkeep, failures, disasters & insurance (**Phase 9 — HRE v1 complete**) | decay: `HRE_DECAY_ARCHETYPE`/`HRE_DECAY_EXPOSURE`/`HRE_CLIMATE_REGION`, `hreBuildQuality`, `hreDecayPerPeriod`, `hreApplyDecay`, compounding via `HRE_DEFER_K`/`HRE_DEFER_CAP_YEARS` (bounded, does not diverge); condition reads `hreConditionScore`/`hreSyncCondition`/`hreConditionFactor`/`hreConditionBand`; state `hreUpkeep`/`hreUpkeepRole`/`hreUpkeepInit`/`hreEnsureUpkeep` (**`s.hre.upk` is bookkeeping ONLY — no component value ever lives on it; decay mutates the crystallised `home.condition` in place, which is what makes R5 structural**); failures `HRE_FAIL_THRESHOLD`/`hreFailureRisk`/`hreEmitFailures`/`HRE_FAIL_STAGES`/`hreFailStage`/`hreFailureTerminal`/`hreRestoreComponent` (ladder `open`→`worsening`→`unfit`, terminal stage **condemns or forces a paid emergency repair** — verified firing, not decorative); disasters `HRE_HAZARDS`/`HRE_HAZARD_REGION`/`HRE_HAZARD_DEFECT`/`hreDisasterCheck`; insurance `HRE_INSURANCE_TIERS`/`hreInsuranceAvailable`/`hreInsuranceQuote`/`hreTakeInsurance`/`hreCancelInsurance`/`hreInsuranceTick`/`hreClaim` (loading per prior claim, refusal beyond `HRE_CLAIM_NEGLECT_YEARS`); maintenance `hreDoMaintenance`/`hreUpkeepUnitCost`/`hreRepairCost`; tick `hreUpkeepTick` (§5982, its own line in `advance()` after `hreOnTick`); menus `hreUpkeepMenu`/`hreInsuranceMenu` + `special: "hreUpkeep"` + the `hreUpkeep` item on `HRE_GROUP` |
...
State: **`s.hre`** `{v: 4, seed, tenure, since, home, recon, mem: {ch, filt}, ten, mtg, upk, ...}` — `HRE_STATE_V` is **4** as of Phase 9, which added the `upk` branch (null until the character is actually responsible for a building; `HRE_STATE_LADDER[3]` is the v3→v4 step, and the repair pass backfills a stripped `upk` on a current-version save). Migration never resets an existing tracker. `seed` is written once and never changed; `mem` (added Phase 6) remembers the player's last-browsed channel/filter. `migrate()` calls `hreMigrate(s)`, which runs a guarded `v`-driven ladder (`HRE_STATE_LADDER`) then a version-independent repair pass — verified against a genuine pre-S06 fixture (`test-hre-s09.js`).
**As of Phase 9 the full arc is player-actionable** — browse, view, apply, rent, be evicted, buy, borrow, maintain, neglect, and lose a home to condemnation or repossession. `HRE_GROUP` carries three items (`hreHome`, `hreMarket`, `hreUpkeep`). **The one thing still not possible is a voluntary sale.** The Phase 6-era note below is kept for historical context: at that time, **browsing was player-visible but nothing beyond browsing was actionable** — no offers, viewings, applications, or moving in (S07/S08, Phase 7+). Both items are `cost: 0`, matching the other 35 menu-opening `special` branches file-wide (see module 01's entry and `PROJECT_CONTEXT.md` — this was verified exhaustive, not HRE-specific, so it wasn't fixed in this patch). Invariants that must not break: no `Math.random`/`rnd`/`pick`/`Date.now` anywhere in S01–S06 (lint-asserted); every generated attribute draws from a *namespaced* sub-stream; law/urban-form/curves are data with no `if (country === ...)` anywhere; valuation is O(1) and cache-independent; **S06's presentation (title/blurb) may only compose from the blueprint's own attributes — it must never invent a fact**, which is also the line module 15's AI narration may not cross; the S09 shim stays equivalent to legacy until Phase 7 inverts call sites.
...
Owns (real files, not in `life-sim.jsx`): `build-slice.py` (auto-exports all **560** top-level symbols — never hand-list exports), `hre-extract.py` (regenerates the standalone `hre-*.js` files from `life-sim.jsx`, the single authority; now extracts **7** sections including `hre-s06.js`; aborts on a duplicated banner, which doubles as a double-splice check), `harness.js` (`mkChar`/`runLife`/`CLASSES`), `hre-p0d.js` (wage-delivery probe — re-run if 09 changes payroll), `hre-sample-review.js` (renders 100 generated properties as prose; the un-automatable Phase 3 gate, and how the amenity-prerequisite bug was found), and the suites: `test-integration-1.js` (53) · `test-stx-verify.js` (69) · `test-stx-wiring.js` (22) · `test-ai-guarantee.js` (19) · `test-render.js` (5) · `test-hre-s01.js` (83) · `test-hre-s03-s04.js` (82) · `test-hre-s02.js` (110) · `test-hre-calib.js` (20) · `test-hre-s05.js` (88) · `test-hre-s09.js` (87, includes explicit v1→v2 ladder coverage added at Phase 6) · `test-hre-s06.js` (60, new at Phase 6 — channels/onset-years, search determinism/bounds, filters, listing shape, disclosure ladder, CE spot checks, Act-sheet wiring incl. the Gotcha #2 discarded-clone check on `s.hre.mem`, zero-dead-end soak, market coverage diagnostic) · `test-hre-s10.js` (**170, new at Phase 9** — R5 handoff continuity, `s.hre.upk` schema + v3→v4 migration, decay by archetype/quality/age/climate, compounding proved via a positive *second difference* in cumulative loss, maintenance spend, failure emission from the live condition model, the decorative-state-machine check on the terminal `unfit` stage, disasters, insurance incl. the neglect-refusal rule, a 4-country multi-decade neglected-vs-maintained value soak, and the menu/display-contract wiring). **868 assertions.**

⚠️ **Only 5 of these files exist in the `logostona/A-Life-v0.44` git repo** — `test-hre-s06.js`, `test-hre-s08b.js`, `test-hre-s09.js`, `test-hre-phase7-gate.js`, `test-hre-s10.js`. `harness.js`, `build-slice.py` and the other 14 suites live only in the sandbox. The repo's `harness.js` is a **reimplementation** from `PROJECT_CONTEXT.md`'s contract, and the repo's `test-hre-s09.js` is the **stale pre-Phase-7 fixture** that reports 8 failures against any current file (verified against the pristine original — not a regression). See `RESTART-BRIEF.md` §2 for the full availability note before trusting any total above.
